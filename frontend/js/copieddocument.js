...
async function getData() {
  if (query0) {
    try {
      const response = await fetch(
        `https://translate.googleapis.com/translate_a/single?client=gtx&sl=vi&tl=en&dt=t&q=${encodeURIComponent(query0)}`
      );
      if (response.ok) {
  ...
  if (query1) {
    try {
      const response = await fetch(
        `https://translate.googleapis.com/translate_a/single?client=gtx&sl=vi&tl=en&dt=t&q=${encodeURIComponent(query1)}`
      );
      if (response.ok) {
        const res = await response.json();
        const datas = res[0];
...
// function getHoveringImagePath(event) {
//   const hoveredImg = event.target;
//   if (hoveredImg.tagName === "IMG") {
//     currentImagePath = hoveredImg.
//     // console.log("Image path:", imgPath);
//     // return imgPath // Do something with the image path
//   }
// }
// 

// document.addEventListener("keydown", function (ev) {
//   if (ev.key === HOTKEY.key && ev.ctrlKey === HOTKEY.ctrl) {
//     ev.preventDefault();
...
//   const scrollContentGrid = (event) => {
//       if (event.key == 'ArrowRight') {
//           event.preventDefault();
//           window.scrollBy(0, 800); //Scroll to the bottom 800px
//       }
//       else if (event.key == 'ArrowLeft') {
//         event.preventDefault();
//         window.scrollBy(0, -800); //Scroll to the top
//     }
//   }

//   document.addEventListener("keydown", scrollContentGrid);
// });
