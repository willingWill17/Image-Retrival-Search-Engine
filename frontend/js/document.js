...
function getData() {
    let res = {query: [{textual: query0, objects: objects0.join(' '), txt: convertPosToStr(pos0)},{textual: query1, objects: objects1.join(' '), txt: convertPosToStr(pos1)}]}
...
canvasBlock0.addEventListener("drop", function (e) {
    ...
    if (draggedElement.tagName.toLowerCase() === 'img') {
        ...
        if(objects0.includes(data)) {
            ...
            pos0[index] = calcGridPos(e.clientX-canvas0.getBoundingClientRect().x, e.clientY-canvas0.getBoundingClientRect().y, data);
        } else {
            pos0.push(calcGridPos(e.clientX-canvas0.getBoundingClientRect().x, e.clientY-canvas0.getBoundingClientRect().y, data));
        }
        post(getData())
...
canvasBlock1.addEventListener("drop", function (e) {
    ...
    if (draggedElement.tagName.toLowerCase() === 'img') {
        ...
        if(objects1.includes(data)) {
            ...
            pos1[index] = calcGridPos(e.clientX-canvas1.getBoundingClientRect().x, e.clientY-canvas1.getBoundingClientRect().y, data);
        } else {
            pos1.push(calcGridPos(e.clientX-canvas1.getBoundingClientRect().x, e.clientY-canvas1.getBoundingClientRect().y, data));
        }
        post(getData())
