...
function loadConfig() {
    promise1 = fetch("config.yaml").then(response => response.text()).then(data => {
            ...
            function(o) {
                if (o.get('type') == 'rect') {
                    if (o && o.oCoords) {
                        ...
                        var endCol = Math.ceil(Math.min(canvasWidth, o.oCoords.tr.x) / cellWidth);
    ...
    try {
        ...
        if (query == "") {
            ...
            $.ajax({
                ...
                data: {
                    query: query,
                    simreorder: simreorder,
                    n_frames_per_row: numResultsPerVideo
                },
                dataType: "text",
